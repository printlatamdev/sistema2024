...
if ($bandera == 1) {
    ...
    $vendedor_id = $_POST['vendedor'];
        ...
        // Asume que ya tienes una conexión a la base de datos en la variable $conn
        ...
        if ($result->num_rows > 0) {
            ...
            // Ahora $vendedor es un array asociativo con las columnas de la tabla vendedores
            $nom_vendedor = $vendedor['nombre']; // o el nombre de la columna que necesites
        } else {
    ...
    $rs = $mysqli->query("INSERT INTO cotizacion (id_contacto, contacto, fecha, nota, id_vendedor, vendedor, id_usuario, id_empresa, empresa, estado) VALUES ( '$contacto', '$nom_contacto', '$fecha', '$nota', '$vendedor', '$nom_vendedor', '$nombre', '$empresa', '$nom_empresa', 'Pendiente')");
    ...
    $detalle = '<br><font color="blue">' . $_SESSION['vsNombre'] . '</font> ha creado Cotizacion Numero: <font color="red"><b>' . $order . '</b></font>.</br>Detalles: Cliente: <b>' . $nom_empresa . '</b>, Contacto: <b>' . $nom_contacto . '</b>, Vendedor: <b>' . $nom_vendedor . '</b>.</br> El dia ' . $date_log . ' a las ' . date('h:i:s a') . ' desde la IP ' . $ip . '.';
    ...
    $base = $_REQUEST['base'];
    ($_SESSION['base'] == '' ? include('../' . $base . '_db.php') : '');
    ...
    $detalle = '<br><font color="blue">' . $_SESSION['vsNombre'] . '</font> ha <font color="red"><b>modificado</b></font> un nuevo detalle de la Cotizacion Numero: <font color="red"><b>' . $orden . '</b></font>.</br></br><b>Detalle:</b><br> ' . $textarea . '<br><br>  <b>Cantidad:</b> <font color="blue"><b>' . $cantidad . '</b></font>, <b>Precio:</b> <font color="blue"><b>' . $precio_unitario . '</b></font></br> El dia ' . $date_log . ' a las ' . date('h:i:s a') . ' desde la IP ' . $ip . '.<br><br>';
    ...
    $detalle = '<br><font color="blue">' . $_SESSION['vsNombre'] . '</font> ha editado la Cotizacion Numero: <font color="red"><b>' . $cot . '</b></font>.
                </br>Detalles:

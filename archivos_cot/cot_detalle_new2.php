...
// Obtener el ID de la orden si está definido
...
if ($nombre == 'admin') {
    ...
    mysqli_stmt_bind_result($stmt, $empresa_nombre);
    mysqli_stmt_fetch($stmt);
    mysqli_stmt_close($stmt);
    $nombre = ($empresa_nombre === "COLGATE PALMOLIVE CA") ? "COLGATE PALMOLIVE CA INC" : $empresa_nombre;
}

// Determinar la consulta a ejecutar
if (empty($_GET['buscar'])) {
    ...
    $query = "
        SELECT DISTINCT t1.id_logistica, t1.id_orden, t1.origen, t1.numorden_compra, t1.f_empaque, t1.p_vehiculo, t1.n_motorista,
               ...
               t1.nota_envio_cd, t1.orden_compra, t1.nota_envio, t1.fo_entrega, t1.estado, t1.descripcion, t2.id_orden,
               t2.id_empresa, t2.empresa, t2.id_contacto, t2.vendedor, t2.contacto, t3.nombre, t3.celular, t3.email,
               t4.id_marca, t4.marca, t5.cot
        FROM logitica t1
    ...
    $ordenes_entregadas = mysqli_num_rows($result);
    mysqli_stmt_close($stmt);
    ...
    $query2 = "
        ...
        ORDER BY a3.cot ASC
    ";
    $stmt2 = mysqli_prepare($conexion2, $query2);
    mysqli_stmt_bind_param($stmt2, 's', $nombre);
    mysqli_stmt_execute($stmt2);
    $result2 = mysqli_stmt_get_result($stmt2);
    $ordenes_entregadas2 = mysqli_num_rows($result2);
    mysqli_stmt_close($stmt2);

    $conteo_entregadas = $ordenes_entregadas + $ordenes_entregadas2;
} else {
    // Consulta cuando hay búsqueda
    $buscar = mysqli_real_escape_string($con, $_GET['buscar']);
    $query = "
        SELECT t1.id_logistica, t1.id_orden, t1.origen, t1.f_empaque, t1.numorden_compra, t1.p_vehiculo, t1.n_motorista,
               t1.destino, t1.status, t1.f_salida, t1.f_llegada, t1.m_carga, t1.carta_p, t1.factura, t1.duca_f, t1.duca_t,
               t1.nota_envio_cd, t1.orden_compra, t1.nota_envio, t1.fo_entrega, t1.estado, t1.descripcion, t2.id_orden,
               t2.id_empresa, t2.empresa, t2.id_contacto, t2.contacto, t3.nombre, t3.celular, t3.email, t4.id_marca, t4.marca
        FROM logitica t1
        INNER JOIN pop t2 ON t1.id_orden = t2.id_orden
        INNER JOIN contacto t3 ON t2.id_contacto = t3.id_contacto
        INNER JOIN pop_marca t4 ON t1.id_marca = t4.id_marca
        WHERE t1.estado = 0 AND t2.id_empresa = ? AND t4.marca LIKE ?
        ORDER BY t1.destino ASC
    ";
    $stmt = mysqli_prepare($con, $query);
    $searchTerm = '%' . $buscar . '%';
    mysqli_stmt_bind_param($stmt, 'is', $id, $searchTerm);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $ordenes_entregadas = mysqli_num_rows($result);
    mysqli_stmt_close($stmt);
    ...
    $query2 = "
        SELECT a1.id_orden, a1.estado, a1.status, a1.marca, a1.origen, a1.destino, a1.f_salida, a1.f_llegada, a1.descripcion,
               a1.m_carga, a1.carta_p, a1.factura, a1.duca_t, a1.duca_f, a1.nota_envio_cd, a1.orden_compra, a1.nota_envio
        FROM logitica a1
        INNER JOIN pop a2 ON a1.id_orden = a2.id_orden
        INNER JOIN empresa a3 ON a2.id_empresa = a3.id_empresa
        WHERE a2.empresa = ? AND a1.estado = 0 AND a1.status = 'ENTREGADO' AND a1.marca LIKE ?
        ORDER BY a1.f_llegada ASC
    ";
    $stmt2 = mysqli_prepare($conexion2, $query2);
    mysqli_stmt_bind_param($stmt2, 'ss', $nombre, $searchTerm);
    mysqli_stmt_execute($stmt2);
    $result2 = mysqli_stmt_get_result($stmt2);
    $ordenes_entregadas2 = mysqli_num_rows($result2);
    mysqli_stmt_close($stmt2);
...
$count_query = "SELECT COUNT(*) AS numrows FROM logitica t1 INNER JOIN pop t2 ON t1.id_orden = t2.id_orden WHERE t2.id_empresa = ?";

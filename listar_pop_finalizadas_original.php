...
if ($nivel==5) {
  $query = mysqli_query($con,"select distinct a.id_orden, a.id_empresa, a.empresa, a.contacto, a.fecha_orden, a.vendedor, a.destino, a.id_proyecto, a.nombre_proyecto, a.estado, a.computo, a.impresion, a.corte, b.id_orden, b.origen,b.status,  b.f_llegada, b.f_salida,c.id_proyecto, c.marca, c.nombre from logitica b inner join pop a on b.id_orden = a.id_orden inner join pop_proyecto c on a.id_proyecto=c.id_proyecto where a.impresion='0' and a.id_orden LIKE '%".$query2."%' OR a.nombre_proyecto LIKE '%".$query2."%' order by a.id_orden desc  ");
}elseif($nivel==15){
...
$query = mysqli_query($con,"select distinct a.id_orden, a.id_empresa, a.empresa, a.contacto, a.fecha_orden, a.vendedor, a.destino, a.id_proyecto, a.nombre_proyecto, a.estado, a.computo, a.impresion, a.corte, b.id_orden, b.origen,b.status, c.id_proyecto, c.marca, c.nombre from logitica b inner join pop a on b.id_orden = a.id_orden inner join pop_proyecto c on a.id_proyecto=c.id_proyecto where a.computo='0' and a.id_orden LIKE '%".$query2."%' OR a.nombre_proyecto LIKE '%".$query2."%' order by a.id_orden desc  ");
  ...
  $query = mysqli_query($con,"select distinct a.id_orden, a.id_empresa, a.empresa, a.contacto, a.fecha_orden, a.vendedor, a.destino, a.id_proyecto, a.nombre_proyecto, a.estado, a.computo, a.impresion, a.corte, b.id_orden, b.origen,b.status, b.f_llegada, b.f_salida, c.id_proyecto, c.marca, c.nombre from logitica b inner join pop a on b.id_orden = a.id_orden inner join pop_proyecto c on a.id_proyecto=c.id_proyecto where a.estado = '0' and  a.id_orden LIKE '%".$query2."%' OR a.nombre_proyecto LIKE '%".$query2."%'  order by a.id_orden desc");
}
...
?>
       ...
       <table class="table table-striped table-hover" style="width: 100%;">
        ...
        <tr>
                  ...
                  <th>Info</th>
                  <th>Data</th>
                  <?php if ($nivel==1 || $nivel==2 || $nivel==3 || $nivel==11 || $nivel==50): ?>
                      <th>Impresiones</th>
                  <?php endif ?>
        ...
        if(mysqli_num_rows($query) == 0){
          ...
          //$archive="browser/elfinder6.php?year=20".$_SESSION['year']."&empresa=".$row['empresa']."&marca=".$row['marca']."&proyecto=".$row['nombre_proyecto']."&nivel=".$nivel."&tipo=".$tipo."&base=".$base;
...
$finales++;
  ...
  ';$nivel=$_SESSION['nivel'];
        ...
        if ($nivel==1 || $nivel==2 || $nivel==3 || $nivel==9  || $nivel==20  || $nivel==7 || $nivel==11 || $nivel==50) { 
          ...
          echo '<td>';
            $sql58 = mysqli_query($con, "select DISTINCT id_orden, cot from pop_detalle where id_orden='".$row['id_orden']."'");
                  while($row52 = mysqli_fetch_assoc($sql58)){
                    $cot=$row52['cot'];
                    if ($cot=="1113-19") {
                      echo'<div class="col-md-12"><a href="cotizaciones_esa19/colgate_palmolive_ca_1113_2019_12_05.pdf" data-fancybox="preview"><span class="label label-info">'.$cot.'</span></a></div>';
                    }
                    if ($cot=="1154-19") {
                      echo'<div class="col-md-12"><a href="cotizaciones_esa19/colgate_palmolive_ca_1154_2019_12_19.pdf" data-fancybox="preview"><span class="label label-info">'.$cot.'</span></a></div>';
                    }
                      ...
                      $query233 = mysqli_query($con,"select*from cotizacion where id_cotizacion='".$cot."'");
                           while($row9 = mysqli_fetch_assoc($query233)){
                              $cot_d=$row9['archivo'];
                                ...
                                echo'<div class="col-md-12"><a href="cotizaciones_'.$cotrut.'/'.$cot_d.'" data-fancybox="preview"><span class="label label-info">'.$cot.'</span></a></div>';
                            }
...
';
        ...
        if ($nivel==1 || $nivel==2 || $nivel==9|| $nivel==17 ||$nivel==11 || $nivel==50) {
...
function <?echo $n_funcion;?>(str) {
  ...
  xmlhttp.open("GET","getuser_fin.php?q="+str,true);
  xmlhttp.send();
}
</script>
    <tr>

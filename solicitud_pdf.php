...
where  t1.id_orden='" . $n_orden . "' order by t1.id_orden desc");
  ...
  id_orden,n_orden,proveedor,flete,entrega_ad,tramite_adu,cancelacion,impuestos,transporte,detalleflete,solicitud_pdf,origen,destino,fecha,estado) VALUES ('" . $n_orden . "','" . $suma . "','" . $proveedor . "','" . $flete . "','" . $entrega . "','" . $aduanal . "','" . $cancelacion . "','" . $impuestos . "','" . $trans . "', '" . $detalleflete . "','" . $namepdf . "','" . $origen . "','" . $destino . "','" . $fecha . "','" . $est . "')");

    //ACTUALIZANDO NUMERO DE SOLICITUD DE DESPACHO DE LA TABLA POP
    $update_soli = $mysqli->query("update pop SET numero_solicitud='" . $suma . "' where id_status='" . $n_orden . "'");
  } else {
...
where  t1.id_orden='" . $n_orden . "' order by t1.id_orden desc");
    ...
    // Suponiendo que $mysqli ya está conectado correctamente a la base de datos

    $res21 = $mysqli->query("INSERT INTO solicitud_despacho (
    id_orden, n_orden, proveedor, flete, entrega_ad, tramite_adu, cancelacion, impuestos, transporte, detalleflete, solicitud_pdf, origen, destino, fecha, estado
) VALUES (
    '$n_orden', '$suma', '$proveedor', '$flete', '$entrega', '$aduanal', '$cancelacion', '$impuestos', '$trans', '$detalleflete', '$namepdf', '$origen', '$destino', '$fecha', '$est'
)");
 ...
 window.location.href="./solicitudes_elsalvador.php";
      ...
      echo json_encode([
        'status' => 'success',
        'message' => 'Solicitud de despacho creada exitosamente.'
      ]);
      ...
      echo json_encode([
        'status' => 'error',
        'message' => 'Error al crear la solicitud de despacho: ' . $mysqli->error
      ]);
    }
    //ACTUALIZANDO NUMERO DE SOLICITUD DE DESPACHO DE LA TABLA POP
    $update_soli = $mysqli->query("update pop SET numero_solicitud='" . $suma . "' where estado='" . $n_orden . "'");
  }
}
...
*/
    ...
    $total4 = $flete + $entrega + $aduanal + $cancelacion;
    $total2 = $flete + $entrega + $aduanal;
    //solo si es para transporte fito
    if ($proveedor == "TRANSPORTE VANEGAS") {
      $iva2 = $total2 * 0.02;
    } else {
      $iva2 = $total2 * 0.15;
      //$iva2= $total2*0.02;
    ...
    $pdf->Cell(1, 6, '', 0, 0, 'L', 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(180, 12, '' . utf8_decode($detalleflete) . '', 1, 0, 'C', 1);
    // $pdf->SetFont('Arial','',10);
    //$pdf->Cell(40,6,'  $'.$TOTALIVA.'',1,0,'C',1);
    //TERMINA DETALLE DEL FLETE 
    ...
    $pdf->Cell(190, 0, "Greddy Estrada", 0, 200, "C");
    $pdf->Cell(320, 0, "Douglas Davila", 0, 200, "C");
    //$pdf -> Cell(360,0,"Lic. Eduardo Campos ",0,200,"C");
    $pdf->ln(3);
    $pdf->Cell(50, 0, "Daysi del Socorro", 0, 200, "C");
...
*/
    ...
    //$pdf -> Cell(360,0,"Lic. Eduardo Campos ",0,200,"C");
    $pdf->ln(3);
    $pdf->Cell(50, 0, "Emerson Flores", 0, 200, "C");
...
*/
    ...
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(26, 6, 'FLETE', 1, 0, 'C', 1);

    $pdf->Cell(62, 6, 'ENTREGA ADICIONAL', 1, 0, 'C', 1);
    $pdf->Cell(62, 6, 'T. ADUANAL', 1, 0, 'C', 1);

    $pdf->Cell(25, 6, 'SUB-TOTAL', 1, 0, 'L', 1);

    $pdf->Ln(7);

    $total4 = $flete + $entrega + $aduanal + $cancelacion + $impuestos;
    $total2 = $flete + $entrega + $aduanal;
    ...
    $iva = bcdiv($iva2, '1', 2);
    $total = bcdiv($total2, '1', 2);
    $pdf->SetFillColor(255, 255, 255);
    $pdf->Cell(26, 10, '$' . $flete, 1, 0, 'C', 0);

    $pdf->Cell(62, 10, '$' . $entrega, 1, 0, 'C', 1);
    $pdf->Cell(62, 10, '$' . $aduanal, 1, 0, 'C', 1);
    //$pdf->Cell(25,10,'$'.$cancelacion,1,0,'C',1); CODIGO ANTIGUO
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(25, 10, ' $' . $total . '', 1, 0, 'C', 1);
    $pdf->Ln(10);
    ...
    $TOTALIVA2 = $total4 + $iva;
    $TOTALIVA = bcdiv($TOTALIVA2, '1', 2);

    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(125, 6, '', 0, 0, 'L', 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(25, 6, '(+) SINI', 1, 0, '', 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(25, 6, '  $' . $cancelacion . '', 1, 0, 'C', 1);
    ...
    $pdf->Cell(125, 6, '', 0, 0, 'L', 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(25, 6, '(+) IMPUESTOS', 1, 0, '', 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(25, 6, '  $' . $impuestos . '', 1, 0, 'C', 1);
    $pdf->Ln(6);
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(125, 6, '', 0, 0, 'L', 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(25, 6, 'TOTAL:', 1, 0, '', 1);
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(25, 6, '  $' . $TOTALIVA . '', 1, 0, 'C', 1);
    ...
    // Configura el archivo PDF antes de generarlo
    $pdf->SetFont("Arial", "", 10);

    // Añade contenido al PDF aquí
// Ejemplo: $pdf->Cell(40,10,'¡Hola, Mundo!');

    // Asegúrate de que todas las configuraciones y el contenido estén listos antes de guardar el PDF
    $carpeta_artes_orden = "logistica/SOLICITUDES DE DESPACHO/";
    ...
    try {
      $pdfFileName = $carpeta_artes_orden . $origen . $guardar . '.pdf';
      // Guardar el PDF
      $pdf->Output('F', $pdfFileName);
      // echo "PDF generado y guardado en: " . $pdfFileName;

    } catch (Exception $e) {
      echo 'Error al generar el PDF: ', $e->getMessage();
    }

    // Después de `Output`, no puedes realizar más cambios en el PDF

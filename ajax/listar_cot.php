...
session_start();
if (!isset($_SESSION)) {
    die("La sesión no se ha iniciado.");
}
...
?>
    ...
    <tbody>
        ...
        while ($row = mysqli_fetch_array($query)) {
            $id_proyecto = htmlspecialchars($row['id_cotizacion']);
            $nombre = strtoupper(htmlspecialchars($row['empresa']));
            $contacto = strtoupper(htmlspecialchars($row['contacto']));
            $usuario = strtoupper(htmlspecialchars($row['id_usuario']));
            $fecha = strtoupper(htmlspecialchars($row['fecha']));
        ...
        ?>
            <tr class="<?php echo htmlspecialchars($text_class); ?>">
                ...
                <td>
                    <a class="edit" data-fancybox data-options='{"type": "iframe", "iframe": {"preload": false}}' href="cot_detalle.php?orden=<?php echo urlencode($id_proyecto); ?>">

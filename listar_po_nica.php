...
<?php
  ...
  /* Connect To Database*/
  require_once ("conexion_ajax_nica.php");
    ...
    if ($numrows>0){
        ...
        if(mysqli_num_rows($query) == 0){
          ...
          while($row = mysqli_fetch_assoc($query)){
                 ...
                 $archive='browser/elfinder122.php?campo='.$row['id_orden'].'&&tipo=campos2&&nivel='.$nivel.'&&base='.$base.'';
        ...
        if ($nivel==1 || $nivel==2 || $nivel==3 || $nivel==9|| $nivel==17) {
...
function <?echo $n_funcion;?>(str) {
  ...
  xmlhttp.open("GET","getuser_op_nica.php?q="+str,true);
  xmlhttp.send();
}
</script>

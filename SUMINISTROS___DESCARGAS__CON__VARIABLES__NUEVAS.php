...
$bandera=$_REQUEST['bandera'];
        $carpeta1 = "ORDENES_COMPRA_2020/bodega_".$_SESSION['base'].$_SESSION['year']."/";
        if (!file_exists($carpeta1)) {
        mkdir($carpeta1, 0777, true);}
        $carpeta2 = "ORDENES_COMPRA_2020/ordenes_compra_".$_SESSION['base'].$_SESSION['year']."/";
        if (!file_exists($carpeta2)) {
        mkdir($carpeta2, 0777, true);}
        $carpeta3 = "ORDENES_COMPRA_2020/orden_compra_docu_".$_SESSION['base'].$_SESSION['year']."/";
        if (!file_exists($carpeta3)) {
        mkdir($carpeta3, 0777, true);}
        $carpeta4 = "ORDENES_COMPRA_2020/ordenes_compra_fac_".$_SESSION['base'].$_SESSION['year']."/";
...
$bandera=$_REQUEST['bandera'];
			...
			$carpeta_envio = "ORDENES_COMPRA_2020/ORDENES_DOCU/";
		    if (!file_exists($carpeta_envio)) {
		    mkdir($carpeta_envio, 0777, true);}
		    $carpeta = "ORDENES_COMPRA_2020/BODEGA_2020/";
...
if ($bandera==1) {
			...
			{ 
							...
							while ($fila = $rs->fetch_assoc()) { $estado=$fila['estado']; $nombre_tinta=$fila['tipo']; }
							              ...
							              //****Bitacora del Sistema*******
						                   ...
						                   $username = $_SESSION['username'];
								            ...
								            if ($empresa_tinta=='nicaragua') {
									           	...
									           	$detalle= "El usuario ".$_SESSION['username']." a descargado la tinta <font color=red>".$codigo."</font> para nicaragua  a las ".date('h:i:s a').".";


											} else {

				                                $detalle= "El usuario ".$_SESSION['username']." a descargado de bodega la tinta <font color=red>".$codigo."</font>  a las ".date('h:i:s a').".";
           ...
           while ($fila = $rs->fetch_row()) { $existencia=$fila[3]; $nombre_material=$fila[2]; $tipo=$fila[1]; }
           ...
           while ($fila = $rs->fetch_row()) { $existencia=$fila[3]; $nombre_material=$fila[2]; $tipo=$fila[1]; }
			              ...
			              //****Bitacora del Sistema*******
				            ...
				            $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
				            $date_log=$dias[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y') ."";
							$detalle= $_SESSION['username']." - Ingreso cantidad:<font color=red>".$cantidad."</font><br> Cantidad Anterior:".$existencia."<br> Cantidad Actual: <font color=blue> ".$nueva_existencia."</font>";
		   ...
		   $cantidad_tinta=$_POST['cantidad_tinta'];
            ...
            function compress($source, $destination, $quality) {
              ...
              if($source_img=='' || empty($source_img) ){  $link_OC= 'Imagen no disponible.';}
            else{

                      $num=rand(0000,9999);
                      $nombre='COMPRO_'.$stud_p[0].'_'.$num.'.jpg';
                      //BORRAR ESTA LINEA Y RESTITUIR LA COMENTARIADA
                      $destination_img = 'ORDENES_COMPRA_2020/BODEGA_2020/'. $nombre;
             ...
             $source_img2 = $_FILES['compro_2']['tmp_name'];
              if($source_img2=='' || empty($source_img2) ){  $link_FAC= 'Imagen no disponible.'; }
            else{
      ...
      //------------------------------------Fin Para reducir foto
           ...
           while ($fila = $rs->fetch_row()) { $codigo=$fila[3];  }
		   ...
		   if(empty($letters)){
		   	 ...
		   	 while ($fila = $rs1->fetch_row()) { $abreviacion=$fila[3];  }
           ...
           for ($x = 1; $x <= $cantidad_tinta; $x++) {
			               ...
			               //****Bitacora del Sistema*******
				            ...
				            $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
				            $date_log=$dias[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y') ."";
							$detalle= $_SESSION['username']." - Ingreso los siguientes codigos de tinta:<br> ".$log_code."<br>Comprobante 1: ".$link_OC."<br>Comprobante 2: ".$link_FAC." ";
           ...
           $log_code="";
		  // echo '1';
		  header("Location: suministros.ingreso.bodega.php"); 
           ...
           $nota=$_REQUEST['nota'];
            ...
            function compress($source, $destination, $quality) {
              ...
              if($source_img=='' || empty($source_img) ){ $orden_compra=''; $link_OC= 'Imagen no disponible.';}
            else{
             ...
             $source_img2 = $_FILES['factura_imagen']['tmp_name'];
              if($source_img2=='' || empty($source_img2) ){ $factura=''; $link_FAC= 'Imagen no disponible.'; }
            else{
      ...
      //------------------------------------Fin Para reducir foto
           ...
           while ($fila = $rs->fetch_row()) { $existencia=$fila[3]; $nombre_material=$fila[2]; $tipo=$fila[1]; $medicion=$fila[6]; }
			              ...
			              //****Bitacora del Sistema*******
				            ...
				            $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
				            $date_log=$dias[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y') ."";
							$detalle= $_SESSION['username']." - Ingreso cantidad:<font color=red>".$cantidad."</font><br> Cantidad Anterior:".$existencia."<br> Cantidad Actual: <font color=blue> ".$nueva_existencia."</font><br>Orden Compra: ".$link_OC."&nbsp;&nbsp;   Factura: ".$link_FAC."<br>Nota: ".$nota;
							$rs=$mysqli->query("INSERT INTO log_bodega( usuario, ip, hora, material, detalle) VALUES ('$username', '$ip', '".date("Y-m-d h:i:s")."', '$tipo - $nombre_material', '$detalle')");
	...
	//$nom_material=$_REQUEST['nom_material'];
           ...
           $link_OC= '<a target="_blank" href="ORDENES_COMPRA_2020/ORDENES_DOCU/'.$orden_compra.'"><b><font>Ver</font></b></a>';
           $factura=$_REQUEST['fac_imagen'];
           $link_FAC= '<a target="_blank" href="ORDENES_COMPRA_2020/ORDENES_FACT/'.$factura.'"><b><font>Ver</font></b></a>';
		   $idc=$_REQUEST['idc'];
		   ...
		   //------------------------------------------------------------------------------------------------------------------------------------Para reducir foto
				 ...
				 function compress($source, $destination, $quality) {
				  ...
				  $source_img = $_FILES['compra_imagen']['tmp_name'];
				  if($source_img=='' || empty($source_img) ){ $compra_imagen=''; $link_CI= 'Imagen no disponible.';}
				else{
		   ...
		   //----------------------------------------------------------
			...
			if ($pos === false) {	
				...
				if($color_tinta=='solvente'){ $color_tinta='flush';    }
				...
				while ($fila3 = $rs3->fetch_row()) { $codigo=$fila3[3];  }
				...
				for ($x = 1; $x <= $cantidad_tinta; $x++) {
							...
							$rs4=$mysqli->query("INSERT INTO material_ingreso( id_compra, id_proveedor, proveedor, material, tipo, cantidad, medicion, orden_compra, factura, fecha, nota, metodo, adicional) VALUES ('$compra', '".$tipo_tinta."', '".$empresa."', '$nom_material', 'Tinta', '$cantidad', 'Tinta', '$orden_compra', '$factura', '".date("Y-m-d h:i:s")."', '$nota', 'Sistema', '$compra_imagen')");
								...
								//****Bitacora del Sistema*******
								 ...
								 $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
								 $date_log=$dias[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y') ."";
								 $detalle= $_SESSION['username']." - Ingreso los siguientes codigos de tinta:<br> ".$log_code."<br>Orden Compra: ".$link_OC."&nbsp;&nbsp;   Factura: ".$link_FAC."<br>Nota: ".$nota;

...
if ($bandera == 1) {
	...
	if ($pop == 1 && $empresa != 'campos') {
		$pop_det = "- POP";
		$pagina = "lista_pop_individual.php?popnumero=";
	} else if ($empresa == "campos") {
		$pop_det = "- CAMPOS";
		$pagina = "lista_campos_individual.php?popnumero=";
	} else if ($empresa == 'color' && $pop != 1) {
		$pop_det = "- OP";
		$pagina = "lista_po_individual.php?popnumero=";
	}

	$target = "_blank";

	$comple_href_inicio = "<a href=";
	$comple_href_fin = ">";
	...
	if ($existencia >= $cantidad) {
		...
		$meses = array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
		$date_log = $dias[date('w')] . " " . date('d') . " de " . $meses[date('n') - 1] . " del " . date('Y') . "";
		$detalle = "El usuario " . $_SESSION['vsNombre'] . " a descargado la cantidad <font color=red>" . $cantidad . "</font> del material <b>" . $tipo . " - " . $nombre_material . "</b> a las " . date('h:i:s a') . ".<br><br>Detalles:<br> Existencia anterior: <font color=red>" . $existencia . "</font><br> Cantidad Descargada: <font color=red>" . $cantidad . "</font><br>Existencia actual: <font color=blue>" . $nueva_existencia . "</font><br> Orden: <font color=red><a href=" . $pagina . $orden . " target=" . $target . " >" . $orden . " " . $pop_det . "</a></font><br>Comprobante: <font color=red>" . $comprobante . "</font><br>Entrega: <font color=red>" . $entrega . "</font><br>Comentarios: <font color=red>" . $nota . "</font><br>Empresa: <font color=red>" . $empresa . "</font><br> ";



		$rsxx = $mysqli->query("INSERT INTO log_bodega( usuario, ip, hora, material, detalle, salida, cantidad, orden, scan, pop) VALUES ('$username', '$ip', '" . date("Y-m-d h:i:s") . "', '$tipo - $nombre_material', '$detalle', '$comprobante', '$cantidad', '$orden', '$scan', '$pop')");

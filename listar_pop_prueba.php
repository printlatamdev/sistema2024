...
if ($action == 'ajax') {
  ...
  if ($nivel == 5) {
    ...
    $query = mysqli_query($con, "select distinct a.id_orden, a.id_empresa, a.empresa, a.contacto, a.fecha_orden, a.vendedor, a.destino, a.id_proyecto, a.nombre_proyecto, a.estado, a.computo, a.impresion, a.corte, b.id_orden, b.origen,b.status, c.id_proyecto, c.marca, c.nombre from logitica b inner join pop a on b.id_orden = a.id_orden inner join pop_proyecto c on a.id_proyecto=c.id_proyecto where a.estado = '1' and  a.empresa LIKE '%" . $query2 . "%'  order by a.id_orden desc  LIMIT $offset,$per_page");
  }
?>
      ...
      if ($numrows > 0) {
        ...
        if (mysqli_num_rows($query) == 0) {
          ...
          while ($row = mysqli_fetch_assoc($query)) {
            ...
            $proyect_info = mysqli_query($con,"SELECT * FROM pop_proyecto WHERE id_proyecto=$id_proyecto");
            $server = $_SERVER['SERVER_ADDR'];


            //
            $archive = "../sistema2024/browser/elfinder6x67.php?year=2023" . "&empresa=" . $row['empresa'] . "&marca=" . $row['marca'] . "&proyecto=" . $row['nombre'] . "&nivel=" . $nivel . "&tipo=" . $tipo . "&base=" . $base . "&orderid=".$row['id_orden'];
              ...
              if ($nivel == 1 || $nivel == 2 || $nivel == 3 || $nivel == 9 || $nivel == 20 || $nivel == 7 || $nivel == 50) {
                ...
                while ($row52 = mysqli_fetch_assoc($sql58)) {
                  ...
                  if ($encontrado == 0 && $encontrado2 == 0) {
                    ...
                    while ($row92 = mysqli_fetch_assoc($result)) {
                      $cot_d2 = $row92['archivo'];
                      echo '<div class="col-md-12"><b><h5><a href="../sistema/cotizaciones_nica20/' . $cot_d2 . '" data-fancybox="preview"><span class="label label-info">' . $cot . '</span></a></div>';
                    }
                    ...
                    while ($row92 = mysqli_fetch_assoc($result)) {
                      $cot_d2 = $row92['archivo'];
                      echo '<div class="col-md-12"><b><h5><a href="../sistema/cotizaciones_nica20/' . $cot_d2 . '" data-fancybox="preview"><span class="label label-info">' . $datos3 . '</span></a></div>';
                    }
                  }
                }
                echo '</td>';
...
';
   ...
   <a data-toggle="collapse" data-target=' . $data . ' aria-expanded="false" aria-controls=' . $data2 . '><img src="imagenes/info.png" width="15"/></a>
            ...
            <?php if ($nivel == 17 || $nivel == 1 || $nivel == 2 || $nivel == 3 || $nivel == 11 || $nivel == 9 || $nivel == 50) : ?>
              <td>
                <div class="col-xs-6">
                  <a data-fancybox data-options='{"data-backdrop":"static", "type" : "iframe", "iframe" : {"preload" : false } }' href="reportes/remision_pop_option.php?base=<?= $base; ?>&id=<? echo $row['id_orden']; ?>" class="btn bg-warning" title="Imprimir Nota de Envio"><i class="fas fa-print"></i></a>
                </div>
  ...
  <script>
    ...
    function <? echo $n_funcion; ?>(str) {
      ...
      // xmlhttp.open("GET", "getuser_new_pop.php?q=" + str, true);

...
a1.carta_p, a1.factura, a1.duca_t, a1.duca_f, a1.nota_envio_cd, a1.orden_compra, a1.nota_envio FROM logitica a1 inner join pop a2 on a1.id_orden=a2.id_orden inner join empresa a3 on a2.id_empresa=a3.id_empresa WHERE a2.empresa='".$nombre."' and a1.estado=1 and a1.status='IMPRESION'  order by a1.f_llegada asc
 ...
 ");
  ...
  $consulta = mysqli_query($conexion,"  select t1.id_logistica, t1.id_orden, t1.origen, t1.f_empaque, t1.p_vehiculo, t1.n_motorista, t1.destino, t1.status, t1.f_salida, t1.f_llegada, t1.m_carga, t1.carta_p, t1.factura, t1.duca_f, t1.duca_t, t1.nota_envio_cd, t1.orden_compra,t1.nota_envio, t1.fo_entrega, t1.estado,t1.descripcion,t2.id_orden, t2.id_empresa, t2.empresa, t2.id_contacto, t2.contacto, t3.nombre, t3.celular, t3.email, t4.id_marca, t4.marca from logitica t1 inner join pop t2 on t1.id_orden=t2.id_orden inner join contacto t3 on t2.id_contacto=t3.id_contacto inner JOIN pop_marca t4 on t1.id_marca=t4.id_marca where t1.estado=1 and t1.status='IMPRESION' and t2.id_empresa='".$id."' and t4.marca LIKE '%".$_GET['buscar']."%'  order by t1.destino asc 
 ");
...
function myFunction2(val) {
    var v =  val;
    window.open("evento_impresionsv.php?id="+ v, "toolbar=yes,scrollbars=yes,resizable=yes,top=1,left=2000,width=2000,height=2000");
}
...
function myFunction5(val) {
    var v =  val;
    window.open("evento_impresionnc.php?id="+ v, "toolbar=yes,scrollbars=yes,resizable=yes,left=2000,width=2000,height=2000");
}
</script>
        ...
        <table class="dropdown notifications-menu">
                            <thead class="bg-primary" >
                              <tr>
                                   ...
                                   <th width="70" align="center"><!--<a href="index.php" target=ver>--></i>#ORDEN<!--</a>--></th>
...
<div  id="ocultar" class="col-lg-12 col-md-12 col-xs-12">
  <iframe name=ver  align="right" boder=1 frameborder="0" style=" top:-200px; position:relative; " width="650" height="600"></iframe>
</div>
  ...
  <script type="text/javascript">
    $(document).on("click",function(e) {
         ...
         var target = $("#ocultar");
            ...
            if (target.is(e.target) && target.has(e.target).length === 1) { 
               window.location.href="index_unido.php";              
            }
     });
  </script>

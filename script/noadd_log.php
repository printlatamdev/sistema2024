...
$nuevo=$mysqli->query("select id_orden from logiticass where id_orden='$id_orden'");
...
$res = $mysqli->query("INSERT INTO logiticass (id_orden,marca,f_empaque,p_vehiculo,n_motorista,destino,origen,status,f_salida,f_llegada,m_carga,carta_p,factura,duca_f,duca_t,nota_envio_cd,orden_compra,nota_envio,fo_entrega,estado,estado_produccion,numorden_compra) VALUES ('".$id_orden."','".$marca."', '', '', '', '".$destino."' , '".$origen."', '".$status."','','','', '','','','','','','','','1','".$produccion."','".$compra."')");
$res2 = $mysqli->query("INSERT INTO bitacora_ss (id_orden,status,f_hora) VALUES ('".$id_orden."','".$status."','".$fecha."')");
...
$produccion=0;
	...
	$res = $mysqli->query("INSERT INTO logiticass (id_orden,id_marca,f_empaque,p_vehiculo,n_motorista,destino, origen,status,f_salida,f_llegada,m_carga,carta_p,factura,duca_f,duca_t,nota_envio_cd,orden_compra,nota_envio,fo_entrega,estado,estado_produccion,numorden_compra) VALUES ('".$id_orden."','".$id_marca."', '', '', '', '".$destino."' , '".$origen."', '".$status."','','','', '','','','','','','','','1','".$produccion."','".$compra."')");
	$res2 = $mysqli->query("INSERT INTO bitacora_ss (id_orden,status,f_hora) VALUES ('".$id_orden."','".$status."','".$fecha."')");
...
if($res==true && $res2==true){
	echo'<script type="text/javascript">
    alert("Iniciando Proceso de envio");
    window.location.href="../noeditar_logistica.php";
    </script>';

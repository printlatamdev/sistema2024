...
$id=$_SESSION['vsIdtemporal'];
 ...
 $obteniendo = mysqli_query($conexion,"select id_empresa, nombre from empresa where id_empresa='".$id."' ");
   while ($filas = mysqli_fetch_array($obteniendo))
                                   {
...
if (!isset($_GET['buscar']) or empty($_GET['buscar'])){
 ...
 t1 inner join pop_detalle t5 on t1.id_orden=t5.id_orden inner join pop t2 on t5.id_orden=t2.id_orden inner join contacto t3 on t2.id_contacto=t3.id_contacto inner JOIN pop_marca t4 on t1.id_marca=t4.id_marca where t1.estado=1 and status='ACABADO'   and t2.id_empresa='".$id."' order by t5.cot desc
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    --><br>
       ...
       <td>ORDENES EN ACABADO:<img height='30px' src="focus/acabado.png" /> </td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  &nbsp;&nbsp;&nbsp;&nbsp;
...
<? echo' <tr div id="'.$data2.'"class="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample" align="center">';?>
         <?php 
                               ...
                               $consulta = mysqli_query($conexion,"select*from foto_log where id_orden='".$ido."' ");
                               $cant=0;
                                while ($row = mysqli_fetch_array($consulta))
                                   {
                                    $foto=$row['foto'];
               ...
               <a href="../sistema2020/ORDENES_POP/EL SALVADOR/<? echo $row['id_orden'];?>/FOTOSLOG_<? echo $row['id_orden']?>/<?php echo $foto; ?>" data-fancybox="preview" data-width="1100" data-height="1000">
                         <?if($cant<7){?>
                     <img  src="../sistema2020/ORDENES_POP/EL SALVADOR/<? echo $row['id_orden'];?>/FOTOSLOG_<? echo $row['id_orden']?>/<?php echo $foto; ?>"  class="zoom img-fluid "  alt="" style="max-width: 135px; max-height: 135px"> 

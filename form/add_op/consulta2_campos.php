...
if (trim($orderid==false)) {
	$pop =$mysqli->query("  SELECT  id_orden from campos order by id_orden desc LIMIT 1 ");
 	...
 	$sql=$mysqli->query("select*from campos_detalle where id_orden='".$id_orden."'");
	
}
else{
	

 	$sql=$mysqli->query("select*from campos_detalle where id_orden='".$orderid."'");
					...
					while($row =  mysqli_fetch_array($sql)){
						...
						echo '
							...
							<td>
								...
								<a href="../../sistema2024/form/nueva_pop/delete_item.php?id='.$row['id_detalle_orden'].'" title="Eliminar" onclick="return confirm(\'Esta seguro de borrar los datos '.$row['nombres'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
							</td>
						</tr>
						';
						$no++;

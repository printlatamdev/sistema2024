...
<div class="table-title">
        ...
        if ($nivel==1 || $nivel==2 || $nivel==3) {?>
            ...
            <a data-fancybox data-options='{"type" : "iframe", "iframe" : {"preload" : false }}' href="formop2.php" class="btn btn-success" ><i class="material-icons">&#xE147;</i> <span>Agregar Nueva Orden</span></a>
          </div>

<?}else{?>


            <a data-toggle="modal" data-target="#no" class="btn btn-success" ><i class="material-icons">&#xE147;</i> <span>Agregar Nueva Orden</span></a>
...
if ($nivel==1) {
 $sql=$mysqli->query("SELECT * FROM tbcomentario WHERE admin = 1 ORDER BY id DESC ");
            $ordenes_activas=mysqli_num_rows($rs1);

 
$ca=mysqli_num_rows($sql);
if ($ca>0) {?>
...
<?
$sql22=$mysqli->query("UPDATE tbcomentario set admin=0 WHERE admin = 1 ORDER BY id DESC ");
 ...
 $sql=$mysqli->query("SELECT * FROM tbcomentario WHERE computo = 1 ORDER BY id DESC ");
            $ordenes_activas=mysqli_num_rows($rs1);
...
<?
$sql22=$mysqli->query("UPDATE tbcomentario set computo=0 WHERE computo = 1 ORDER BY id DESC ");
...
elseif ($nivel==5) {
 $sql=$mysqli->query("SELECT * FROM tbcomentario WHERE impresion = 1 ORDER BY id DESC ");
            $ordenes_activas=mysqli_num_rows($rs1);
...
<?
$sql22=$mysqli->query("UPDATE tbcomentario set impresion=0 WHERE impresion = 1 ORDER BY id DESC ");
...
elseif ($nivel==15) {
 $sql=$mysqli->query("SELECT * FROM tbcomentario WHERE corte = 1 ORDER BY id DESC ");
            $ordenes_activas=mysqli_num_rows($rs1);
...
<?
$sql22=$mysqli->query("UPDATE tbcomentario set corte=0 WHERE corte = 1 ORDER BY id DESC ");

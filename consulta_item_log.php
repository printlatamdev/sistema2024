...
if ($nivel==5) {
  ...
  $obteniendo2 = mysqli_query($conexion,"select * from pop_detalle where id_orden='".$row['id_orden']."' and impresion=1");
 
}
elseif($nivel==15){

 $obteniendo2 = mysqli_query($conexion,"select * from pop_detalle where id_orden='".$row['id_orden']."' and corte=1");

}

elseif($nivel==4){

 $obteniendo2 = mysqli_query($conexion,"select * from pop_detalle where id_orden='".$row['id_orden']."' and computo=1");
...
else
{
  ...
  $obteniendo2 = mysqli_query($conexion,"select * from pop_detalle where id_orden='".$row['id_orden']."'");
}
...
<?$n_funcion='s'.$row['id_orden'].'(this.value)';
 ...
 <button  type="button" id="'.$button.'" class="btn btn-success" data-toggle="collapse" data-target='.$dataa.' aria-expanded="false" aria-controls='.$dataa2.' value='.$id_detalle.' onclick='.$n_funcion.' style="width:235px;" ><font size="1.7">ITEM # '.$n_item.": ".$nombre.'</button>&nbsp;&nbsp;';  

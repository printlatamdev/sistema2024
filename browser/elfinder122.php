	...
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			$(document).ready(function() {
				$('#elfinder').elfinder({
					url : 'php/connector.minimal812.php?empresa=<?=$empresa?>&marca=<?=$marca?>&proyecto=<?=$proyecto?>&nivel=<?=$nivel?>&year=<?=$year?>&tipo=<?=$tipo?>&campo=<?=$campo?>&base=<?=$base?>',  // connector URL (REQUIRED)
				    lang: 'es',
				    height: '600px'

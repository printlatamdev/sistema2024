	...
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			$(document).ready(function() {
				console.log('Out Of ElFinder');
				$('#elfinder').elfinder({
					url : 'php/connector.minimal832.php?empresa=<?=$empresa?>&marca=<?=$marca?>&proyecto=<?=$proyecto?>&nivel=<?=$nivel?>&year=<?=$year?>&tipo=<?=$tipo?>&base=<?=$base?>&orderid=<?=$orderid?>',  // connector URL (REQUIRED)
				    lang: 'es',
				    height: '600px',
				});
